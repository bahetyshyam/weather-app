...
class WeekWeather extends React.Component {
    ...
    getWeeksWeather = async() => {
        ...
        const response = await weatherAPI.get('/forecast', {
            params : {
                ...
                lon : location.long,
                units : 'metric',
                APPID : '688d7763a128fcef39d3dad3e90b5ac8'
            }
        });
        ...
        for(const [index,dayForecast] of responseData.list.entries()) {
            if(index % 8 === 0) {
                const desc = dayForecast.weather[0].description.replace(/\w+/g, function(w){return w[0].toUpperCase() + w.slice(1).toLowerCase();});
                weeksWeather.push({
                    id:index,
    ...
    render() {
        const {weeksWeather} = this.state;
        return <div className="ten wide column">{weeksWeather ? this.renderTable() : 'Loading'}</div>
    }
}
